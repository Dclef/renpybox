...
| 官方进程控制 | `RenpyExtractor.official_extract` 使用 Popen，0.5 秒轮询、900 秒默认超时、取消并转发新输出；主版本探针优先 | 只终止直接进程；静默时无心跳；输出仍累计；回调异常时进程收尾不完整；探针失败仍猜测版本 |
| 增量 70% 分离 | 简单 strings 逐行筛选、行内取消和心跳；混合 TL 先做文本命中筛选 | 文件列表仍全量枚举排序，混合 TL 整体 AST 内不能取消，无统一累计阶段指标 |
