...
> 2026-09-15 新增 DarkMagic 解包、反编译完成后界面停留的现场记录，见第 10 节；主线程状态扫描已后台化，真实游戏复测仍待完成。
...
- 本轮原始调查时没有用户出问题的游戏目录与 API 时序日志。2026-09-15 已获得 DarkMagic 的路径、日志、截图及现场进程信息，见第 10 节；仍缺真实 API 时序和停顿时调用栈，尚未确认该游戏每次卡死的唯一原因。
...
| UI 与应用 | 已有后台角色扫描、事务应用和增量缓存迁移；本次旧 TL 递归计数、应用前枚举和步骤二三次脚本状态检查也移入后台 | 同步路径解析、配置落盘和顶层目录非空检查仍在；不能声称所有 UI 磁盘操作已移出 |
...
7. **步骤二状态扫描后台化**：初次检查、解包完成复检和反编译完成复检统一使用 `GameStatusWorker`；扫描支持取消，按项目键和抽取代数丢弃过期结果，状态检测复用一次可剪枝的脚本遍历。
...
| P2 | 统一运行时/编码能力及解包/反编译产物校验 | `call_game_python`、解包/反编译、OneKey 页面 | 探针未知明确报告；Python 2/3、Ren'Py 7/8、非 UTF-8 不静默损坏；生成源码通过匹配运行时解析后再清理 `.rpyc` |
| 外部验收 | 固定游戏副本与真实 API 语料对照 | 运行记录、游戏日志、吞吐报告 | 固定存档和界面检验覆盖、启动时间；固定 1,000 条语料比较有效唯一条/分钟与质量，再决定批量/并发默认值 |

### 9.4 验证入口

本次新增回归：UI 文件扫描、步骤二状态扫描、预算迁移/设置/请求参数、Hook 分片失败恢复及诊断。当前惰性分块、动态索引和 UI 回归说明见 [实验附录第 9 节](translation-performance-benchmarks.md)。整合抽取/增量恢复/请求/缓存/翻译调度回归为 **372 项通过、6 项跳过**；最后调整初始化预检后，预算/快照 **32 项通过**，新增 Hook 诊断 **12 项通过**（这些组与整合运行部分重叠，不相加计算总数）。步骤二状态扫描新增慢扫描心跳和取消回归；跳过项为 Windows 当前环境无法创建符号链接的测试；已有 scipy/qfluentwidgets 弃用警告。三份文档的 UTF-8、代码围栏和相对链接检查通过，`git diff --check` 通过。

本次验证未调用翻译 API、未运行实际游戏；只能确认步骤二状态扫描不再占用 UI 线程，不能据此断言 DarkMagic 的所有停顿已解决。

## 10. 2026-09-15：解包、反编译完成后界面仍停留

**状态：主线程同步扫描路径已修复；尚未证明它是本次所有停顿的唯一原因。** 本节现场证据来自 `dev` 的 `b393826`，当前实现另有后台状态扫描改动。本次只读检查用户已运行的进程和日志，没有主动启动游戏、调用翻译 API 或修改程序代码。

### 10.1 用户现象与现场证据

游戏为 DarkMagic v0.22.0，目录为 `D:\renpy\DarkMagic-v0.22.0-pc\DarkMagic-V.0.22.0-pc`。用户反馈一键步骤 2 在解包和反编译时都会长时间停留，截图分别显示“正在解包 RPA 归档”和“正在使用匹配版本的 unrpyc 反编译”。

- 2026-09-14 23:57:07：归档扩展安全校验报 `index_archives returned empty`，随后执行 UnRen 直接解包。
- 23:58:22：日志记录三个归档全部解包完成。`module/Tool/Packer.py:494` 的完成日志在子进程退出、退出码检查及清理之后打印，因此不能把该日志之后的等待解释为 UnRen 仍未解包结束。
- 23:59:20 开始反编译；23:59:44 日志记录“反编译后已清理 74 个同名 .rpyc”（现场日志 `log/app.log.2026-09-14`）。结合后续进程状态，本次已越过反编译阶段。
- 后续只读进程检查发现，RenpyBox 的子进程正在使用游戏自带的 `lib/py2-windows-x86_64/python.exe` 执行 `DarkMagic.py <游戏目录> translate chinese`，说明检查当时已经进入官方文本提取。

截图和进程检查不是同一时刻的采样，不能据此断言官方提取运行时界面仍显示反编译，也不能证明官方提取已经死锁。安全校验警告中的“跳过 UnRen 兜底”只禁用 `.bat` 兜底；后续自带工具的直接解包是另一条路径，两条日志并不矛盾。校验失败原因尚未单独复现。

### 10.2 已确认的共同阻塞路径

历史代码中，解包完成回调 `OneKeyTranslatePage._on_auto_unpack_finished` 和反编译完成回调 `_on_auto_decompile_finished` 在 UI 主线程同步调用 `_detect_game_status`，步骤 2 初次进入时也使用同一检测入口。当前三条入口统一提交 `GameStatusWorker`，主线程只处理完成信号。

历史版本的 `_detect_game_status` 调用 `source_script_counts`；`module/Renpy/ProjectPaths.py` 当时分别对整个 `game` 目录执行 `rglob("*.rpy")`、`rglob("*.rpyc")`，TL 排除发生在结果过滤时，并没有阻止遍历进入这些目录。当前 `source_script_counts` 已改为一次 top-down `os.walk`，在目标 TL 目录处剪枝；大量资源目录或慢磁盘仍会放大扫描耗时，但扫描已不在 UI 线程执行。

因此，历史版本中**解包和反编译本身虽然在后台，完成后的脚本检查仍会占用 UI 线程**。当前版本已将检查移到后台，并在扫描期间显示“检测游戏状态”；慢扫描心跳回归确认主线程仍可处理事件。真实游戏的停顿栈仍未采集，不能将本次改动写成永久死锁的唯一修复。

官方文本提取是后续独立阶段：已有超时和取消轮询，但静默进程缺少持续心跳。最初的进程快照只能证明当时提取尚未退出；用户随后补充的完整日志已确认本次因源码解析错误退出，见第 10.4 节，不再将本次官方提取列为原因未知的持续等待。

### 10.3 已完成的修复与剩余验收

此项已按第 9.3 节的源码状态扫描任务完成代码修复；以下第 1–4 项已在本轮实现或回归，第 5 项仍需真实游戏复测：

1. 已将步骤 2 初次检查、解包完成后检查、反编译完成后检查统一移到 `GameStatusWorker`；保留项目/语言代数校验、防重入及取消，避免旧任务结果推动新项目流程。
2. 已将脚本递归统计合并为一次可剪枝遍历；状态检测仍保持原有脚本判定语义，不凭目录名排除游戏资产。
3. 已在扫描开始时显示“检测游戏状态”；解包、反编译、扫描和官方提取分别更新状态。官方提取的静默进程心跳仍是独立待办。
4. 已用受控慢扫描验证 Qt 心跳、取消和页面退出后的旧结果丢弃；流程测试覆盖初次、解包后和反编译后三条入口，完成后只推进一次。
5. 仍需在游戏独立副本复测，分别记录解包退出、反编译退出、脚本扫描起止和官方提取起止；如仍停顿，再采集对应线程栈与进程输出，确认剩余原因。

本节区分代码修复与真实游戏验收：后台状态扫描已经实施，但不能据此声称真实游戏卡顿全部解决。

### 10.4 后续日志：Python 2 游戏官方抽取因源码解析错误退出

用户补充的 2026-09-15 日志明确了后续结果：

| 时间 | 阶段与结果 |
| --- | --- |
| 00:00:16 | 20%，使用游戏自带 `lib/py2-windows-x86_64/python.exe` 执行 `translate chinese` |
| 00:00:51 | 官方解析 `game/corruptions.rpy:767` 失败，退出码 1；提示前面的 `at` 语句不接受缩进块 |
| 00:00:51 | 自动降级为仅补充抽取，进度 50% |
| 00:01:09 | 补充抽取去重完成 |
| 00:01:42 | 添加 820 条静态翻译条目，进入过滤阶段 |
| 00:01:50 | 清理及内置 UI 翻译注入结束，显示 100% 抽取完成 |

**本次官方抽取失败已确认，但不能归结为“Python 2 项目不能官方抽取”或“选错了 Python”。** 命令使用的是游戏自带 Python 2；直接失败原因是游戏解析器拒绝当前磁盘源码。只读查看报错位置得到：
...
```renpy
                    at transform:
                        ...
                        pause 0.075
                        yoffset 20
                        ease 0.75 yoffset 0 alpha 1
```

其中 `at transform:` 位于第 766 行，错误指向第 767 行。该文件修改时间为 2026-09-14 23:59:40，处于这次反编译期间；本工具按 Python 2 选择 `unrpyc_python_v1`，其 `decompiler/sl2decompiler.py:590` 确实会生成 `at transform:`。因此优先调查**反编译输出与游戏自带 Ren'Py 语法的兼容性**。仍需对照原始归档内的脚本/编译产物及游戏实际 Ren'Py 版本，才能确认是反编译还原错误、版本不匹配还是原有源码问题；不要直接批量改缩进或替换所有 `at transform:`。

另外，`remove_decompiled_rpyc`（`module/Tool/RenpyDecompiler.py:25`）目前只根据同名 `.rpy` 是否存在就清理 `.rpyc`，没有先验证生成源码能被游戏解析。本次日志显示已清理 74 个同名 `.rpyc`，随后官方解析失败，说明“反编译进程成功/生成了 RPY”不足以作为源码可用的验收标准。

补充待办（P1）：

1. 用最小 Python 2 / 对应 Ren'Py 版本夹具覆盖本例 screen 中的 `at transform` 和 `has frame` 嵌套，比较反编译前后的解析结果，在独立副本复现并修复兼容性。
2. 调整反编译产物校验与清理顺序：保留可恢复的原始编译产物，先确认生成源码可由匹配的游戏运行时解析，再决定清理；失败时明确展示文件、行号和恢复方式。
3. 官方抽取失败后即使补充抽取完成，也应在最终结果中持续显示“官方抽取失败，仅完成补充抽取”及首个解析错误，不能只显示成功的 100%。820 是本轮新增静态条目数，不是完整覆盖率证明，也不代表所有官方对白已补齐。

以上为新增证据与待修复项。本次未修改游戏脚本、反编译器或抽取逻辑，尚未完成兼容性复现和修复。
