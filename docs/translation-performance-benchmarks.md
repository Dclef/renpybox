...
`test_onekey_file_scans.py` 使用受控后台等待验证：步骤二脚本状态扫描和旧 TL 扫描未完成时 Qt timer 仍执行；状态扫描可取消，页面退出后不消费旧结果；计数保留全量/增量选择；清空路径、换语言或项目后不使用旧结果；应用枚举可取消和防重入。它不提供磁盘速度或真实 UI P95。
