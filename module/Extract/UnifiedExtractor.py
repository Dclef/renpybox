...
class UnifiedExtractor:
    ...
    def _is_builtin_ui_file(self, path: Path) -> bool:
        """检查是否为 RenpyBox 自带的 UI 模板文件。

        ``common.rpy`` and ``screens.rpy`` under ``tl/<language>`` are normally
        generated by Ren'Py's official extractor and must remain visible to
        duplicate detection and cleanup.  Only the files shipped in
        ``base_box`` (and their ``*_box.rpy`` names) are tool-owned templates.
        """
        try:
    ...
    def _iter_rpy_files(self, tl_dir: Path):
        """遍历 tl 目录下的 rpy 文件，跳过 RenpyBox 自带模板。"""
        files = sorted(
            tl_dir.rglob("*.rpy"),
    ...
    def _dedupe_string_translations(self, tl_dir: Path, tl_name: str) -> int:
        ...
        try:
            ...
            if removed_blocks:
                self.logger.info(
                    f"字符串唯一性校验后：已移除 {removed_blocks} 个空的 translate strings 块"
                )
            return removed
        except Exception as exc:
