...
class RenPyArchive:
    ...
    def __init__(self, file = None, version = 3, padlength = 0, key = 0xDEADBEEF, verbose = False):
        ...
        self.files = {} # Initialize here to be safe
        self.indexes = {}
    ...
    def read(self, filename):
        ...
        if filename in self.files:
            ...
            if isinstance(content, tuple) and len(content) == 2 and content[0] == "__PATH__":
    ...
    def add_file_path(self, filename, path):
        """Add a file by path, reading it only when saving."""
        filename = self.convert_filename(_unicode(filename))
        if filename in self.files or filename in self.indexes:
            raise ValueError('file {0} already exists in archive'.format(_printable(filename)))
        
        self.verbose_print('Adding file path {0} to archive...'.format(_printable(filename)))
        self.files[filename] = ("__PATH__", path)
    ...
    def save(self, filename = None):
        ...
        for file, content in files.items():
            ...
            if isinstance(content, tuple) and len(content) == 2 and content[0] == "__PATH__":
                path = content[1]
                current_len = os.path.getsize(path)
            ...
            # Update index.
            if self.version == 3:
                indexes[file] = [ (offset ^ self.key, current_len ^ self.key) ]
            elif self.version == 2:
                indexes[file] = [ (offset, current_len) ]
            offset += current_len
