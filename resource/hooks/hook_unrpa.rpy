init python early hide:
    ...
    # 自动删除 Hook 文件（解包完成后清理）
    current_file_path = os.path.abspath(sys.argv[0])
    current_dir_path = os.path.dirname(current_file_path)
    game_dir = os.path.join(current_dir_path, 'game')
    hook_files = [
        os.path.join(game_dir, 'hook_unrpa.rpy'),
        os.path.join(game_dir, 'hook_unrpa.rpyc'),
    ]
    for hook_file in hook_files:
        try:
            if os.path.isfile(hook_file):
                os.remove(hook_file)
                print('已删除 Hook 文件: ' + hook_file)
        except Exception as e:
            print('删除 Hook 文件失败: ' + hook_file + ' - ' + str(e))